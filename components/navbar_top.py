...
def NavbarBusqueda(titulo, icono):
    ...
    def Mark_as_read(id_notification):
        ...
        response = requests.put(
            url + "update-status/" + id_notification, headers=headers
        )
        response.raise_for_status()
        datos = response.json()
        print(datos)
    ...
    def list_Notifications():
        ...
        for notification in list_notifications["notifications"]:
            notifications_user.append(
                html.li(
                    {"style": {"list-style": "none"}},
                    html.div(
                        html.p(notification["Mesaage"]),
                        html.p(notification["_id"]),
                        html.button(
                            {
                                "class": "btn btn-primary",
                                "on_click": lambda event: Mark_as_read(
                                    notification["_id"]
                                ),
                            },
                            "Marcar como leido",
                            html.i({"class": "bi bi-check2-square me-1 ms-1"}),
                        ),
                    ),
                    html.hr(),
                )
        ...
        html.div(
            html.button(
                {
                    ...
                    "on_click": lambda event: request_data(event),
                },
                html.i(
                    {
            ...
            html.div(
                ...
                html.div(
                    ...
                    html.h5(
                        {"class": "offcanvas-title", "id": "offcanvasRightLabel"},
                        "Notificaciones",
                    ),
                    html.button(
                        {
                            ...
                            "aria-label": "Close",
                            "on_click": lambda event: setNotifications({}),
                        }
                    ),
                ),
                html.div({"class": "offcanvas-body"}, show_notifications()),
            ),
        ),
    )
